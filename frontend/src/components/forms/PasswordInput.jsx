import React, { useState } from 'react';

/**
 * Reusable password input component with hide/show toggle functionality
 * 
 * @param {string} id - Input id attribute
 * @param {string} name - Input name attribute
 * @param {string} value - Input value
 * @param {function} onChange - Change handler
 * @param {function} onBlur - Blur handler (optional)
 * @param {string} placeholder - Placeholder text
 * @param {boolean} required - Whether field is required
 * @param {string} autoComplete - Autocomplete attribute
 * @param {string} className - Additional CSS classes for the input
 * @param {string} containerClassName - Additional CSS classes for the container
 * @param {boolean} hasError - Whether there's an error (for border styling)
 * @param {string} minLength - Minimum length attribute
 * @param {boolean} autoFocus - Whether input should autofocus
 * @param {boolean} disabled - Whether input is disabled
 * @param {string} inputMode - Input mode attribute
 */
const PasswordInput = ({
  id,
  name,
  value,
  onChange,
  onBlur,
  placeholder = 'Enter password',
  required = false,
  autoComplete = 'current-password',
  className = '',
  containerClassName = '',
  hasError = false,
  minLength,
  autoFocus = false,
  disabled = false,
  inputMode,
}) => {
  const [showPassword, setShowPassword] = useState(false);

  const togglePasswordVisibility = () => {
    setShowPassword(!showPassword);
  };

  // Default styling for the input
  const defaultInputClass = `w-full px-3 py-2 border rounded-md bg-white dark:bg-gray-700 text-gray-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent ${
    hasError ? 'border-red-500 dark:border-red-500' : 'border-gray-300 dark:border-gray-600'
  }`;

  // Default styling for the container
  const defaultContainerClass = 'relative';

  return (
    <div className={`${defaultContainerClass} ${containerClassName}`}>
      <input
        type={showPassword ? 'text' : 'password'}
        id={id}
        name={name}
        value={value}
        onChange={onChange}
        onBlur={onBlur}
        required={required}
        autoComplete={autoComplete}
        placeholder={placeholder}
        minLength={minLength}
        autoFocus={autoFocus}
        disabled={disabled}
        inputMode={inputMode}
        className={`${defaultInputClass} ${className} pr-10`}
      />
      <button
        type="button"
        onClick={togglePasswordVisibility}
        className="absolute inset-y-0 right-0 flex items-center pr-3 text-gray-500 dark:text-gray-400 hover:text-gray-700 dark:hover:text-gray-200 focus:outline-none"
        tabIndex={-1}
        aria-label={showPassword ? 'Hide password' : 'Show password'}
      >
        {showPassword ? (
          // Eye-off icon (hide password)
          <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M13.875 18.825A10.05 10.05 0 0112 19c-4.478 0-8.268-2.943-9.543-7a9.97 9.97 0 011.563-3.029m5.858.908a3 3 0 114.243 4.243M9.878 9.878l4.242 4.242M9.88 9.88l-3.29-3.29m7.532 7.532l3.29 3.29M3 3l3.59 3.59m0 0A9.953 9.953 0 0112 5c4.478 0 8.268 2.943 9.543 7a10.025 10.025 0 01-4.132 5.411m0 0L21 21" />
          </svg>
        ) : (
          // Eye icon (show password)
          <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15 12a3 3 0 11-6 0 3 3 0 016 0z" />
            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z" />
          </svg>
        )}
      </button>
    </div>
  );
};

export default PasswordInput;
